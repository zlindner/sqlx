...
impl<S: Socket> BufferedSocket<S> {
    ...
    async fn read_buffered(&mut self, len: usize) -> io::Result<BytesMut> {
    ...
    pub async fn flush(&mut self) -> io::Result<()> {
        while !self.write_buf.is_empty() {
            let read = self.socket.write(self.write_buf.get()).await?;
            self.write_buf.consume(read);
        }
