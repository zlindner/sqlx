...
pub trait Socket: Send + Sync + Unpin + 'static {
    ...
    // NOTE: no `flush()` because this trait is designed purely for TCP and UDS sockets
...
pub async fn connect_uds<Ws: WithSocket>(path: &str, with_socket: Ws) -> crate::Result<Ws::Output> {
    if cfg!(not(unix)) {
